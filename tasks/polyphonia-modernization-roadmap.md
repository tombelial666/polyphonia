...
- **2026-04-24:** usability/runtime slice для Assist и Claude launcher: в `phrygian_app.py` исправлен запуск Claude terminal для frozen `polyphonia.exe` через реальный Python launcher с fallback на прямой `claude`; затем launcher переведён в более совместимый Windows-режим без принудительного `TERM=xterm-256color` и с `CLAUDE_CODE_DISABLE_TERMINAL_TITLE=1`, чтобы не провоцировать TUI input freezes у native `claude.exe`; поверх этого `scripts/claude_terminal.py` переведён в режим **stable API REPL first** — если Claude key доступен из настроек/окружения, открывается простой управляемый terminal chat через Anthropic Messages API, а если ключа ещё нет, terminal сам запрашивает одноразовый Claude API key прямо в консоли вместо сломанного OAuth/TUI flow; native Claude Code оставлен только как явный fallback через `--native`; в `build_index.py` и `assets/polyphonia_ui.js` обновлены настройки Claude key (remember/forget + связь с launcher), а чат получил развёрнутый focus/fullscreen-style режим, более читаемую историю, статусную строку и GPT-style кнопки Copy с clipboard fallback; обновлена заметка `docs/dev-notes/claude-terminal-launcher.md`; проверки: `pytest -q tests/test_build_index.py tests/test_phrygian_window_management.py tests/test_claude_terminal_launcher.py`, `python build_index.py`, `python -m PyInstaller --noconfirm polyphonia.spec`.
