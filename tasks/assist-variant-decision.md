...
- **2026-04-23:** MusicXML export в Assist теперь, помимо браузерной загрузки, сохраняет локальную копию через `pywebview` API `save_musicxml`; реализация в `assets/polyphonia_ui.js` и `phrygian_app.py`, покрытие добавлено в `tests/test_phrygian_openai_connection.py`.
...
- **2026-04-23:** добавлен режим **«Запомнить на этом ПК»** для ключей OpenAI (и инфраструктура под Claude): сохранение в `polyphonia_sessions/auth.json` (gitignored), автоподхват при старте, кнопка «Забыть», новые API `set_openai_api_key_persist` / `forget_openai_api_key_persisted`, тесты в `tests/test_phrygian_openai_connection.py`.
- **2026-04-23:** улучшен запуск терминала Claude Code из UI: при открытии отдельного окна выставляется `chcp 65001` (UTF-8) и `TERM=xterm-256color` для более стабильного TUI-ввода/выбора в Windows.
