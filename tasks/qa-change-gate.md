...
- **2026-04-23 (Hotfix: detach + key validation + Claude cmd):** `phrygian_app.py` — `validate_openai_key` разрешён в offline режиме (для сплэша), `open_assist_window` пересоздаёт окно если оно закрыто и пытается вывести на передний план, добавлен `get_repo_root`; `assets/polyphonia_ui.js` — копирование команды Claude теперь предпочитает `cd /d "<repo>" && python scripts/claude_terminal.py`.
- **2026-04-23 (Coverage baseline поднят):** `build_index.py` стал импортируемым (вынесено `build_index_html()` + `main()`), добавлен `pytest-cov` в `requirements-dev.txt`, добавлены unit‑тесты `tests/test_build_index.py` и `tests/test_change_gate_index_parsing.py`, расширен `tests/test_claude_terminal_launcher.py`, добавлен ретро‑лог `docs/dev-notes/agent-error-log.md`, обновлён `repo-index.yaml`.
- **2026-04-23 (Hotfix: client log + Claude окно):** `phrygian_app.py` — `log_client_event` больше не падает из-за конфликта `_log(event=...)`; `launch_claude_terminal_window` теперь делает preflight (наличие `claude` в PATH), показывает понятные ошибки в отдельной консоли и сообщает `missing_launcher` если запуск не из корня репозитория.
